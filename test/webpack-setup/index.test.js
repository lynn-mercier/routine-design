...
describe('render-server/WebpackSetup', function() {
  const webpackSetup = new WebpackSetup('./tmp', './tmp/index.js', Application);
  it('#emptyDirectory', async function() {
    const promise = webpackSetup.emptyDirectory(rimraf, fs).then(() => {
      expect(td.explain(rimraf).calls[0].args[0]).to.deep.equal('./tmp');
      expect(td.explain(fs.mkdirSync).calls[0].args[0]).to.deep.equal('./tmp');
    })
  ...
  it('#write', async function() {
    ...
    return webpackSetup.write('./tmp/routes.js', EntryPoint).then(() => {
      expect(td.explain(EntryPoint.prototype.write).calls[0].args[0]).to.equal('./tmp');
      expect(td.explain(Application.prototype.writeHtml).calls[0].args[0]).to.equal('./tmp/index.html');
      expect(td.explain(EntryPoint.prototype.write).calls[0].args[1]).to.equal('./tmp/routes.js');
      expect(td.explain(EntryPoint.prototype.write).calls[0].args[2]).to.equal('./tmp/index.js');
  ...
  it('#startServer', async function() {
    ...
    const promise = webpackSetup.startServer(1234, Webpack, WebpackDevServer).then(() => {
      expect(td.explain(Application.prototype.createConfig).calls[0].args[0]).to.equal("./tmp/index.js");
      ...
      expect(td.explain(WebpackDevServer).calls[0].args[1].contentBase).to.equal('./tmp');
