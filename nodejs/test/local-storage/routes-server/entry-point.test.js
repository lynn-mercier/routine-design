...
const EntryPoint = require('../../../src/local-storage/routes-server/entry-point');

describe('local-storage/routes-server/EntryPoint', function() {
  //TODO make tmp dir
  ...
  it('#write(routesPath, webpackDir, javaScriptPath)', async function() {
    const promise = entryPoint.write('./tmp', './tmp/routes.js', './tmp/index.js', mockFs).then(() => {
      ...
      expect(td.explain(mockFs.writeFile).calls[1].args[1]).to.equal(fs.readFileSync('test/local-storage/routes-server/golden.js', 'utf8'));
    });
